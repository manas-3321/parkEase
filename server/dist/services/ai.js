...
class AIService {
     ...
     */
    static async verifyParkingImage(imageUrl, spaceName) {
     ...
     */
    static async predictAvailability(spaceId, targetTime, durationHours) {
        ...
        for (const event of events) {
            ...
            if (targetTime >= start && targetTime <= end) {
                // Distance check
                const dist = AIService.calculateDistance(space.latitude, space.longitude, event.latitude, event.longitude);
                if (dist <= 1.5) {
                    hasNearbyEvent = true;
     ...
     */
    static async calculateDynamicPrice(spaceId) {
        const space = await db_1.default.parkingSpace.findUnique({
            where: { id: spaceId },
        });
        if (!space) {
        ...
        const explanations = [`Base price: ₹${space.pricePerHour.toFixed(2)}/hour`];
        let multiplier = 1.0;
        ...
        const activeBookingsCount = await db_1.default.booking.count({
            where: {
